...
stopifnot(all(with(overlap_o, table(species, id_method, id_year, month, id_fishing_group)) %in% 0:1))
```
...
sra_ini <- list(log_q_intercept = rep(-10, sra_dat$n_method),
                ...
                p_net_capture_z = rep(0.5, sra_dat$n_species_group))
```
...
```{r}
# save
usethis::use_data(fishing_years,  overwrite = TRUE)
...
sra_mdl <- rstan::stan_model(file = "../inst/extdata/stan/sra_v0.0.3.stan")
...
tab <- data.frame(id = 1:71, species = species_groups$common_name, predicted_captures = round(tmp$par$observed_captures_s), predicted_dead_captures = round(tmp$par$observed_dead_captures_s), risk = round(tmp$par$risk_ratio_s, 2)) %>% left_join(emp, by = "id") %>% dplyr::select(id, species, observed_captures, predicted_captures, observed_dead_captures, predicted_dead_captures, risk)
