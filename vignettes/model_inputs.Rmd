...
```{r}
# use net captures from recent fishery groups only
#use_net_capture_groups <- c(19, 21, 23, 25, 27, 28, 29, 31, 33)
#
#overlap_o <- overlap_o %>% mutate(use_captures_net = case_when(use_captures_net == 1 & !(id_fishing_group %in% use_net_capture_groups) ~ -1L, TRUE ~ use_captures_net))
#
# check this one
#overlap_o %>% filter(id_fishing_group == 28 & n_captures_net > 0)
```
