...
sra_dat <- list(n_species = length(species),
            n_month = 12,
            n_area = 1L, #length(unique(overlap_t$area));
            ...
            area_j          = rep(1L, nrow(overlap_t)), #overlap_t$area,
...
rm(gg, df1, cdead2, cdead1, clive2, clive1, cdat1, cdat2)
...
tab <- data.frame(id = 1:71, species = species_groups$common_name, captures = round(tmp$par$observed_captures_s), dead_captures = round(tmp$par$observed_dead_captures_s), risk = round(tmp$par$risk_ratio_s, 2)) %>% left_join(emp, by = "id") %>% dplyr::select(id, species, observed_captures, captures, observed_dead_captures, dead_captures, risk)
