...
sra_dat <- list(n_species = length(species),
            ...
            n_capture_type = 3 + 1,
            n_capture_type_group = length(unique(species_groups$species_group)),
            ...
            capture_type_group_s = species_groups$id_species_group,
...
tmp <- rstan::optimizing(object = model, data = sra_dat, init = sra_ini, verbose = TRUE, as_vector = FALSE)
...
#sra_fit <- rstan::sampling(object = model, data = sra_dat, chains = 2, iter = 300, warmup = 100, thin = 1, init = function() sra_ini, verbose = TRUE)
#usethis::use_data(sra_fit, overwrite = TRUE)
```

```{r mcmc_2, fig.cap = "MCMC trace", fig.height = 5}
#x <- rstan::extract(sra_fit, pars = paste0("traces[", 5:7, "]"), permute = FALSE)
#
#dimnames(x) <- list(iteration = 1:dim(x)[1], chain = paste0("chain_", 1:dim(x)[2]), parameter = c("Prob. live capture", "N breeding pairs", "Prob. breeding"))
#x <- reshape2::melt(x)
#
#ggplot(x) + geom_line(aes(iteration, value, col = chain)) + facet_grid(parameter~., scales = "free_y") + theme_bw(base_size = 14) + theme(axis.text.y = element_blank(), axis.title.y = element_blank()) #+ guides(col = "none") + xlab("Iteration")
```
