...
transformed parameters {
...

  {
      ...
      for (i in 1:n_i) {
        ...
        // (set equal to 1 for non-trawl captures)
        p_net_capture_i[i] = use_net_captures_i[i] > 0 ? p_net_capture_s[species_i[i]] : 1.0;
        
        // conditional probability of being live given capture
        // (for the trawl fishery this is the probability of being
        // live given net capture, because only net captures
        // are observed alive)
...
model {
  ...
  // assumed to be net captures meaning 
  // that p(live) = p(live|net) x p(net) 
  for (i in 1:n_i) {     
      if (captures_i[i] > 0) {
          live_captures_i[i] ~ binomial(captures_i[i], p_live_capture_i[i] * p_net_capture_i[i]);
      }
  }
} // end of model
generated quantities {
  ...
  // from total overlap
  {
    ...
    for (j in 1:n_j) {
        ...
        // probability that capture is a net capture
        // (equal to 1 for non-trawl fishery groups)
        p_net_capture_j = use_net_captures_j[j] > 0 ? p_net_capture_s[species_j[j]] : 1.0;
        
        // observable death
        observable_deaths_j = captures_j * (1 - p_live_capture_zg[species_group_j[j], fishery_group_j[j]] * p_net_capture_j);
