...
data {
  ...
  int<lower=-1> net_captures_i[n_i];
...
transformed parameters {
  ...
  vector[n_i] p_live_capture_i;
  vector[n_i] p_net_capture_i;
  ...
  vector<lower=0>[n_i] mu_observed_captures_net_warp_i;
...

  {
      ...
      for (i in 1:n_i) {
        ...
        mu_observed_captures_net_warp_i[i] = net_captures_i[i] >= 0 ? mu_observed_captures_i[i] * (net_captures_i[i] + warp_captures_i[i]) / captures_i[i]: 0.0;
        
        p_net_capture_i[i]  = p_net_capture_z[species_group_i[i]];
        ...
        mu_net_captures_i[i]  = mu_observed_captures_net_warp_i[i] * p_net_capture_i[i];
        mu_warp_captures_i[i] = mu_observed_captures_net_warp_i[i] * (1.0 - p_net_capture_i[i]);
      }
  }
...
generated quantities {
  ...
  int observed_captures_net_warp_i[n_i];
      ...
      for(s in 1:n_species){
        for(g in 1:n_fishery_group){
            ...
            if (idx_method_fg[g] == 4) {
                ...
                vulnerability_sg[s,g] += q_sg[s,g] * (1 - p_live_capture_sg) * p_net_capture_z[species_group_s[s]] * cm;
                
                cryptic_multiplier_sg[s,g] += (1 - p_live_capture_sg) * p_net_capture_z[species_group_s[s]] * cm;
                
                // Warp
                cm = cc_warp_par1[s] > 0.0 ? lognormal_rng(log(cc_warp_par1[s]), cc_warp_par2[s]) : 1.0;
                
                vulnerability_sg[s,g] += q_sg[s,g] * (1 - p_live_capture_sg) * (1.0 - p_net_capture_z[species_group_s[s]]) * cm;
                
                cryptic_multiplier_sg[s,g] += (1 - p_live_capture_sg) * (1.0 - p_net_capture_z[species_group_s[s]]) * cm;
  ...
  
  {
      ...
      for (s in 1:n_species) {
        ...
        theta[3] = 1.0;
        lambda_max[1] = algebra_solver(solve_lambda_di, y_guess, theta, x_r, x_i)[1];
  ...
  for (i in 1:n_i) {
    ...
    observed_captures_net_warp_i[i] = 0;
    ...
    if (mu_observed_captures_i[i] > 0) {
        ...
        if (observed_captures_i[i] > 0) {
            ...
            observed_live_captures_i[i] = binomial_rng(observed_captures_i[i], p_live_capture_i[i]);
            observed_dead_captures_i[i] = binomial_rng(observed_captures_i[i], 1.0 - p_live_capture_i[i]);
            
        }
        
    }
    
    if (mu_observed_captures_net_warp_i[i] > 0) {
        
        observed_captures_net_warp_i[i] = poisson_rng(mu_observed_captures_net_warp_i[i]);
        
        if (net_captures_i[i] >= 0) {
            
            observed_net_captures_i[i]  = binomial_rng(observed_captures_net_warp_i[i], p_net_capture_i[i]);
            observed_warp_captures_i[i] = binomial_rng(observed_captures_net_warp_i[i], 1.0 - p_net_capture_i[i]);
        }
  ...
  // from total overlap
  {
    ...
    int  deaths_j;
      ...
      for (j in 1:n_j) {
        ...
        p_live_capture_j = p_live_capture_zg[species_group_j[j], fishery_group_j[j]];
        ...
        if (mu_captures_j > 0) {
            ...
            dead_captures_j = binomial_rng(captures_j, 1.0 - p_live_capture_j);
        ...
        if (mu_interactions_j > 0) {
            ...
            deaths_j   = binomial_rng(interactions_j, 1.0 - p_live_capture_j * p_survive);
