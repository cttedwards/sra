...
data {
  ...
  int idx_trawl_fg[n_fishery_group];  // index vector equal to 1 for trawl fishery groups
...
transformed parameters {
  ...
  vector<lower=0,upper=1>[n_fishery_group] p_net_capture_zg[n_species_group];
  ...
  // net capture probability per fishery group
  {
      int k;
      for (z in 1:n_species_group) {
          
        p_net_capture_zg[z] = rep_vector(0.0, n_fishery_group);
        
        k = 1;
        for (g in 1:n_fishery_group) {
            if (idx_trawl_fg[g] > 0) {
                
                p_net_capture_zg[z,g] = inv_logit(p_net_capture_z[z] + p_net_capture_g[k]);
                k += 1;
            } 
        }
      }
...

  {
      ...
      for (i in 1:n_i) {
        ...
        p_net_capture_i[i] = use_net_captures_i[i] > 0 ? p_net_capture_zg[species_group_i[i], fishery_group_i[i]] : 1.0;
...
model {
  ...
  // priors on net capture coefficients
  p_net_capture_z ~ std_normal();
  p_net_capture_g ~ std_normal();
...
generated quantities {
      ...
      for(s in 1:n_species){
        for(g in 1:n_fishery_group){
            ...
            if (idx_method_fg[g] == 4) {
                ...
                vulnerability_sg[s,g] = q_sg[s,g] * (p_net_capture_zg[species_group_s[s],g] * cc_net + (1.0 - p_net_capture_zg[species_group_s[s],g]) * cc_warp);
                
                cryptic_multiplier_sg[s,g] = p_net_capture_zg[species_group_s[s],g] * cc_net * (1 - p_live_capture_sg * omega) + (1.0 - p_net_capture_zg[species_group_s[s],g]) * cc_warp;
  ...
  traces[11] = vector_norm(append_row(p_net_capture_z, p_net_capture_g));
